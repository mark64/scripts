...
int main(const char **argv, int argc) {
  ...
  if (pid < 0)
    ...
    return 0;
    //		execlp("gpg", "gpg", "--export", "--armour", "0xF6D1D9B4AA30D5AA",
    //">", filename, (char *) NULL); 		execlp("ssh-keygen", "ssh-keygen", "-q",
    //"-t", type, "-b", bitstring, "-f", filename, (char *) NULL);
  } else {
    int exit = 0;
    waitpid(pid, &exit, 0);
    ...
    if (exit == 0) {
      ...
      if (keyfile < 0) goto key_open_error;
      ...
      for (int i = 0; (max = pread(keyfile, keybuf, QRSIZE, i * QRSIZE)); i++) {
        ...
        QRcode *code =
            QRcode_encodeString(keybuf, 0, QR_ECLEVEL_M, QR_MODE_8, 1);
